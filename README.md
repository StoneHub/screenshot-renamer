...
**No ongoing app:** it starts only when macOS saves a screenshot, renames that screenshot, and exits.
...
After installation, use your normal screenshot shortcut. Each new screenshot is renamed automatically after macOS finishes writing it. App names are inferred only from visible image content. No foreground-app inspection. Existing screenshots are not scanned or renamed.

The installer detects your current macOS screenshot destination. If you have screenshots set to save in Documents, Dropbox, or another folder, it attaches there automatically. To choose a dedicated folder instead, edit the first setting in [config.py](config.py), set an absolute path, and run `install.py` again:

```py
SCREENSHOT_DIRECTORY = "/Users/you/Pictures/Screenshots"
```
...
- Folder Actions and permission for the automation to access your screenshot destination. macOS may request Automation access to System Events.
...
Folder Actions are macOS's file-saved trigger mechanism for this workflow. The helper receives each file macOS adds to the configured screenshot destination, confirms Apple's screenshot metadata before asking the model, then exits. It does not scan the folder on startup. A cancelled/incomplete save or delayed/missed event can leave the standard name. A user rename during generation prevents the old path from being renamed. Model output is a fallible description; no screenshot text is executed.
